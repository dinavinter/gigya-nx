/******/ (function (modules) {
  ...
  /******/ return __webpack_require__(
    (__webpack_require__.s = './src/legacy/others/gigya.saml.bose.js')
  );
  /******/
})(
  /************************************************************************/
        ...
        (function (gigya) {
          ...
          (function (boot) {
            ...
            function ensureJsSdkLoaded(
              ...
              if (gigya['isGigya']) {
                if (boot.debug) console.debug('$G gigya-jsSdk already loaded.');
                if (!gigya['isReady']) {
            ...
            function loadGigyaJS(
              ...
              var isSecurePage =
                location.protocol.toLowerCase().indexOf('https') == 0;
              var server;
              ...
              if (boot.debug) console.debug('$G loading gigya-jsSdk...');
              document
            ...
            function waitForGigyaServiceReady(done) {
              ...
              window['onGigyaServiceReady'] = function () {
                ...
                if (boot.debug) console.debug('$G gigya-jsSdk loaded!');
                done();
              };
            }
            function ensureBasePluginLoaded(
              ...
              ensureJsSdkLoaded(
                ...
                function () {
                  if (
                    gigya['_']['plugins']['BasePlugin'] &&
                    !gigya['_']['plugins']['BasePlugin']['isMock']
                  ) {
                    done();
                  } else {
          ...
          (function (proxy) {
            ...
            (function (utils) {
              ...
              function openIframe(url, done, timeout) {
                ...
                gigya.utils.script.triggerResource(
                  url,
                  loadHandler,
                  gigya.utils.script.ResourceTypes.iframe,
                  false
                );
              ...
              function redirectToSamlEndPont(endPoint, params, apiKey) {
                ...
                var url =
                  'https://' + domain + '/saml/v2.0/' + apiKey + '/' + endPoint;
              ...
              function saveContext(apiKey, context, done) {
                ...
                gigya.utils.sessionCache.get(
                  key,
                  sessionStorageTimeout,
                  function (savedContexts) {
        ...
        (function (gigyaSaml) {
          ...
          (function (proxy) {
            ...
            function findConfiguration() {
              ...
              var configJson = scriptElement.innerHTML.replace(
                /^\s+|\s+$/g,
                ''
              );
            ...
            function parseConfiguration(unparsedConfig) {
              var scriptConf = proxy.utils.deserializeJSON(
                unparsedConfig.configJson
              );
              if (!scriptConf)
          ...
          (function (proxy) {
            function processLogin(action, configuration) {
              proxy.utils.isLoggedIn(function (loggedIn) {
                if (loggedIn)
                  proxy.utils.redirectToSamlEndPont(
                    'idp/sso/continue',
                    {
                      samlContext: action.samlContext,
                      loginToken: proxy.utils.getLoginToken(
            ...
            function processAfterLogin(action, configuration) {
              proxy.utils.loadContext(
                configuration.apiKey,
                function (savedContext) {
                  action.samlContext =
                    action.samlContext || savedContext.samlContext;
            ...
            function processPassiveLogin(action, configuration) {
              proxy.utils.isLoggedIn(function (loggedIn) {
                var loginToken = loggedIn
                  ? proxy.utils.getLoginToken(configuration.apiKey)
                  : null;
                proxy.utils.redirectToSamlEndPont(
                  'idp/sso/continue',
                  {
              ...
              var onLogoutResponse = function (response) {
                ...
                if (response.samlContext) {
                  var callFinalize = function () {
                    return proxy.utils.redirectToSamlEndPont(
                      'idp/slo/finalize',
                      {
                  ...
                  var done = function () {
                    return window.setTimeout(callFinalize, singleSPTimeout);
                  };
                  if (configuration.logoutURL)
                    proxy.utils.openIframe(
