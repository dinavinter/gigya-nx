...
function processLogin(action, configuration) {
    utils_1.isLoggedIn(function (loggedIn) {
        var _a;
        if (loggedIn) {
            var loginTokenExp = (_a = action.params["loginTokenExp"]) !== null && _a !== void 0 ? _a : utils_1.getLoginTokenExpiration(configuration.apiKey);
...
function processForceLogin(action, configuration) {
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        return tslib_1.__generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, window.gigya._.apiAdapters.web.tokenStore.remove()];
                case 1:
                    _a.sent();
...
function processCancel(action, configuration) {
    action.params && action.params.statusCode !== undefined ? encodeURIComponent(action.params.statusCode) : null;
    utils_1.redirectToSamlEndPont('idp/sso/cancel', {
        samlContext: action.samlContext,
        statusCode: action.params && action.params.statusCode !== undefined ? encodeURIComponent(action.params.statusCode) : null,
        cancelCode: action.params && action.params.cancelCode != undefined ? encodeURIComponent(action.params.cancelCode) : null
    }, configuration.apiKey);
}
    ...
    var onLogoutResponse = function (response) {
        ...
        if (response.samlContext) {
            ...
            if (configuration.logoutURL)
                utils_1.openIframe(configuration.logoutURL, callFinalize, singleSPTimeout);
            else
                callFinalize();
        }
    };
        ...
        return tslib_1.__generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    url = "https://" + domain + "/saml/v2.0/" + apiKey + "/" + endPoint;
...
function getLoginTokenExpiration(apiKey) {
    // returns current login token directly from the web adapter
    return window.gigya._.apiAdapters.web.tokenStore.getGltexpCookie(apiKey);
}
...
function saveContext(apiKey, context, done) {
    ...
    var savedContexts = window.gigya.utils.sessionCache.get(key, sessionStorageTimeout) || [];
...
function isLoggedIn(callback, apiKey) {
    if (hasSession(apiKey)) {
        window.gigya.accounts.getAccountInfo({ callback: function (response) {
                callback((response === null || response === void 0 ? void 0 : response.errorCode) === 0);
            } });
    }
    else
        callback(false);
